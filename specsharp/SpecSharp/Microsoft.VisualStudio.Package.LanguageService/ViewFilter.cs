...
namespace Microsoft.VisualStudio.Package {
    ...
    public class ViewFilter : IVsTextViewFilter, IVsTextViewEvents, IOleCommandTarget, IDisposable {
        ...
        public virtual int GetDataTipText(TextSpan[] aspan, out string textValue) {
            ...
            if (span.iEndLine == this.quickInfoLine && span.iEndIndex == this.quickInfoIdx) {
                ...
                this.source.BeginParse(span.iEndLine, span.iEndIndex, new TokenInfo(), ParseReason.QuickInfo, this.textView, new ParseResultHandler(HandleQuickInfoResponse), false);
                return (int)NativeMethods.E_PENDING;
            }
        ...
        public int TypeChar(char ch) {
            ...
            int rc = 0;
            try {
                rc = ExecCommand(ref VsMenus.guidStandardCommandSet2K, (uint)VsCommands2K.TYPECHAR, 0, pvaChar, IntPtr.Zero);
            } finally {
                ...
                default:
                    ...
                    if (rc < 0 && nextTarget != null) {
                        try {
                            ...
                            Trace.WriteLine("Unhandled exception in IOleCommandTarget.Exec: " + ex.Message);
#else
                        } catch (Exception) {
#endif
                            return rc;
