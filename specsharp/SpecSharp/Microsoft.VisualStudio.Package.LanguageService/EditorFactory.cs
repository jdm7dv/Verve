...
namespace Microsoft.VisualStudio.Package {
    ...
    public class EditorFactory : IVsEditorFactory {
        ...
        public Guid GetRegisteredEditor(string extension) {
            string key = extension.ToLowerInvariant();
            EditorInfo ei = (EditorInfo)GetEditors(this.site)[key];
            if (ei != null) {
                return ei.guid;
            }
            return Guid.Empty;
        ...
        public virtual int CreateEditorInstance(uint createDocFlags, string moniker, string physicalView, IVsHierarchy pHier, uint itemid, IntPtr existingDocData, out IntPtr docView, out IntPtr docData, out string editorCaption, out Guid cmdUI, out int cancelled) {
            ...
            if (!string.IsNullOrEmpty(moniker)) {
                string ext = VsShell.GetFileExtension(moniker);
                docData = IntPtr.Zero;
                docView = IntPtr.Zero;
                editorCaption = null;
                ...
                if (takeover && !isOurs && !isUserDefined) {
