...
//      if (node == null && changes == null){Debug.Assert(false); return;}
//      if (node != null){
//        this.currentDocument = node.SourceContext.Document;
//        if (changes == null){
//          //Node is being deleted
//          if (node.SourceContext.Document != null)
//            node.SourceContext.Document.Update(node.SourceContext.StartPos, node.SourceContext.EndPos-node.SourceContext.StartPos, "");
//        }else{
//          //Node content is being replaced with content of changes
//          if (node.SourceContext.Document != null && changes.SourceContext.Document != null)
//            node.SourceContext.Document.Update(node.SourceContext.StartPos, node.SourceContext.EndPos-node.SourceContext.StartPos, 
//              changes.SourceContext.SourceText);
//        }
//        //Shift node down or up in the source depending on changes to previous nodes
//        node.SourceContext.StartPos += this.currentSourcePositionDelta;
//        node.SourceContext.EndPos += this.currentSourcePositionDelta;
//        //Remove the length of the node from the delta. If the node is being deleted, all subsequent nodes will now shift up.
//        this.currentSourcePositionDelta -= node.SourceContext.EndPos - node.SourceContext.StartPos;
//      }
//      if (changes != null){
//        if (node == null){
//          //changes is being inserted into the target tree. It should already have the right source offset.
//          //Make sure it points to the current document.
//          node.SourceContext.Document = this.currentDocument;
//        }
//      }//else
//        //The node is being deleted, which requires no further action
    }
