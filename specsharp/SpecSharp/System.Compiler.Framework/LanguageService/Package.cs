...
namespace System.Compiler {
  ...
  public abstract class Package : IVsPackage, IVsInstalledProduct,
    ...
    /// </summary>
    /// <returns></returns>
    public virtual LanguageService CreateLanguageService(ref Guid guid) {
      Type type = Type.GetTypeFromCLSID(guid, true);
      if (type != null) {
        System.Reflection.ConstructorInfo ci = type.GetConstructor(new Type[0]);
        if (ci != null) {
          LanguageService svc = (LanguageService)ci.Invoke(new object[0]);
          return svc;
        }
      }
      return null;
        ...
        if (key != null) {
          foreach (string ext in key.GetSubKeyNames()) {
            using (RegistryKey exKey = key.OpenSubKey(ext)) {
              ...
              if (guid is String && (string)guid == languageGuid) {
                string ex = ext.Trim();
    ...
    public virtual int QueryService( ref Guid sid, ref Guid iid, out IntPtr obj ) {
      ...
      try {
        ...
        if (this.languageServices.Contains(key)) {
          LanguageService languageService = (LanguageService)this.languageServices[key];
