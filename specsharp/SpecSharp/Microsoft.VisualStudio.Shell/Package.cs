...
namespace Microsoft.VisualStudio.Shell {
        ...
        IVsToolWindowFactory
    {
        ...
        protected virtual object GetAutomationObject(string name) {
            ...
            foreach(Attribute attr in attributes) {
                ...
                if (pa != null && pa.SupportsAutomation) {
                    ...
                    //
                    if (string.Compare(pa.CategoryName, nameParts[0], true, CultureInfo.InvariantCulture) != 0) {
                        continue;
                    }

                    if (string.Compare(pa.PageName, nameParts[1], true, CultureInfo.InvariantCulture) != 0) {
                        continue;
                    }
        ...
        protected virtual void Initialize() {
            ...
            // Verify if a registry key was specified to disable the "crash
            // on unhandled exceptions in winforms winproc" behavior.
            // The registry key is based on the current AppID's registry hive
            // so for Whidbey with default hive, this would be:
            // [HKLM\Software\Microsoft\VisualStudio\8.0\]
            // (DWORD)CatchUnhandledException = 1
            // Under normal conditions, this registry key should not be set
            // 
            bool throwException = true;
            using (RegistryKey key = this.ApplicationRegistryRoot)
            {
                if (key != null)
                {
                    int disable = 0;
                    object result = key.GetValue("CatchUnhandledException", 0);
                    if (result is int)
                        disable = (int)result;
                    if (disable != 0)
            ...
            // Set exception mode so that unhandled exception in winforms handler
        ...
        public IVsOutputWindowPane GetOutputPane(Guid page, string caption) {
            ...
            int hr;
            try {
                hr = outputWindow.GetPane(ref page, out pane);
        ...
        /// </summary>
        /// <param name="t">Type corresponding to the Service being requested</param>
