...
namespace Bio.IO.SFF
{
    ...
    public sealed class SFFParser : ISequenceParser
    {
        ...
        public SFFParser(string dataSource)
        {
            Open(dataSource);
        }
        ...
        public void Open(string dataSource)
        {
            ...
            using (new StreamReader(dataSource)) {/* not used */}

            _dataSource = dataSource;
        }

        /// <summary>
        /// Parses the sequences out of the file.
        /// </summary>
        /// <param name="streamReader">Stream to read from</param>
        /// <returns>Set of sequences</returns>
        public IEnumerable<ISequence> Parse(StreamReader streamReader)
        {
            using (BinaryReader reader = new BinaryReader(streamReader.BaseStream))
            {
                var header = ParseHeader(reader);
                for (long index = 0; index < header.NumberOfReads; index++)
                {
                    yield return ParseOne(header, reader);
                }
            }
        }
        ...
        /// <summary>
        /// Parses out our sequence file
        /// </summary>
        /// <returns></returns>
        public IEnumerable<ISequence> Parse()
        {
            return Parse(new StreamReader(_dataSource));
        }
        ...
        {
            get { return Properties.Resources.SFF_Description; }
        }
        ...
        public string Name
        {
            get { return Properties.Resources.SFF_Name; }
        }
        ...
        {
            get { return Properties.Resources.SFF_Extensions; }
        }
