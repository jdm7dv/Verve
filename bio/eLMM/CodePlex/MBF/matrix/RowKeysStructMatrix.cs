        ...
        Tuple<Stream, TextReader> ThreadLocalStreamAndTextReader
        {
            get
            {
                if (null == _threadLocalStreamAndTextReader.Value)
                {
                    ...
                    var pair = Tuple.Create(stream, textReader);
                    _threadLocalStreamAndTextReader.Value = pair;
                    ThreadLocalStreamAndTextReaderBag.Add(pair);
                }
        ...
        protected void GetInstanceFromRowKeysStructFileNameInternal(string rowKeysStructFileName, ParallelOptions parallelOptions, FileAccess fileAccess = FileAccess.Read, FileShare fileShare = FileShare.Read)
        {
            lock (this)
            {
                ...
                using (TextReader textReader = File.OpenText(rowKeysStructFileName))
                {
                    ...
                    if (null == colKeysLineOrNull) throw new MatrixFormatException("Surprised by empty file. " + rowKeysStructFileName);
            ...
            lock (this)
            {
                using (FileStream fileStream = File.Open(denseStructFileName, FileMode.Open, fileAccess, fileShare))
                {
                    using (TextReader textReader = new StreamReader(fileStream))
                    {
                        ...
                        Helper.CheckCondition(null != firstLineOrNull, "Surprised by empty file. " + denseStructFileName);
                        Helper.CheckCondition(!firstLineOrNull.StartsWith(FileUtils.CommentHeader), "Comments are not supported in RowKeysAnsi and related files");
                    }
                }
                ...
                while (true)
                {
                    ...
                    {
                        throw new MatrixFormatException(string.Format("The rowkey {0} appears more than once", rowKey));
                    }
