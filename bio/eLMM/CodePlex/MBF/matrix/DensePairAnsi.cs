...
#pragma warning restore 1591
        ...
        {
            Helper.CheckCondition(storeList.Count == colCount, "Assert: There should be one value in the storeList for every colKey");
...
#pragma warning disable 1591
        protected override List<UOPair<byte>> StringToStoreList(string valString)
#pragma warning restore 1591
        {
            if (valString.Length != ColCount * 2)
            {
            ...
            {
                if (valString[i + i].CompareTo(valString[i + i + 1]) == 1)
                {
                    new MatrixFormatException("The char pairs in the densePairAnsi file must be sorted");
                }
                storeList.Add(new UOPair<byte>((byte)valString[i + i], (byte)valString[i + i + 1]));
...
#pragma warning restore 1591
        {
            Helper.CheckCondition(val.Length == 2, "Every val must be two characters. " + val);
        ...
        /// </summary>
        public static UOPair<char> StaticMissingValue = new UOPair<char>('?', '?');
#pragma warning disable 1591
...
#pragma warning restore 1591
        ...
        /// For DensePairAnsi the missing value is always the same: ?,?. This is a static version of that missing value.
        /// </summary>
        public static UOPair<byte> StaticStoreMissingValue = new UOPair<byte>((byte)'?', (byte)'?');
#pragma warning disable 1591
        ...
        internal static void MergeDensePairAnsiFiles(string inputDensePairAnsiPattern, string outputDensePairAnsiFileName, byte[] map, ParallelOptions parallelOptions)
        {
            ...
            foreach (string fileName in FileUtils.GetFiles(inputDensePairAnsiPattern, zeroIsOK:false))
            {
                ...
                using (TextReader textReader = File.OpenText(fileName))
                {
                    ...
                    Helper.CheckCondition(null != header, "expect file to have header");
                    int colKeyCountBefore = colKeyList.Count;
                    string[] headerFields = header.Split('\t');
                    colKeyList.AddRange(headerFields.Skip(1));
                    Helper.CheckCondition(colKeyCountBefore + headerFields.Length - 1 == colKeyList.Count, "Expect no overlap between rowKeys");
                    ...
                    while (null != (line = textReader.ReadLine()))
                    {
                        ...
                        Helper.CheckCondition(fields.Length == 2, "expect two fields");
