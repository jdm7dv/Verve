...
#pragma warning disable 1591
        override public IEnumerable<RowKeyColKeyValue<string, string, TValue>> RowKeyColKeyValues
#pragma warning restore 1591
        {
            get
            {
...
#pragma warning restore 1591
        ...
        public static IEnumerable<string> RowKeysInFile(string denseStructFileName)
        {
            ...
            {
                string header = textReader.ReadLine();
                Helper.CheckCondition(null != header, "input file must contain a first line");
                string[] columns = header.Split(new char[] { '\t' }, 2);
                Helper.CheckCondition(columns.Length > 0 && columns[0] == "var", "The first column in the file should be 'var'");

                string line;
                while (null != (line = textReader.ReadLine()))
                {
                    string[] fields = line.Split('\t');
                    Helper.CheckCondition(fields.Length == 2, "Every var line should have exactly one tab. " + denseStructFileName);
        ...
        private static string SplitVarLine(string line, string denseFileNameForErrorMessage, int colCount, out string valueString)
        {
            ...
            if (fields.Length != 2)
            {
                throw new MatrixFormatException("Every var line should have exactly one tab. " + denseFileNameForErrorMessage + ", line starts with " + fields[0]);
            }
            string rowKey = fields[0];
