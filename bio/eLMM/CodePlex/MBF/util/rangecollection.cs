...
// This is getting rather unweildy. Maybe replace the ints with Element objects that can do some work themselves?
namespace Bio.Util
{
    ...
    public class RangeCollection : ISet<long>, IXmlSerializable
    {
        ...
        public long FirstElement
        {
            get
            {
                Helper.CheckCondition<ArgumentOutOfRangeException>(_startItems.Count > 0, "RangeCollection is empty.");
        ...
        {
            Helper.CheckCondition(_rangeExpression.IsMatch(rangeAsString), rangeAsString + " is not a valid range. Must be of the form m-n or m or 'empty' (or 'null').");
            Match match = _rangeExpression.Match(rangeAsString);
            long begin = long.Parse(match.Groups["begin"].Value);
            long last = match.Groups["last"].Value == "" ? begin : long.Parse(match.Groups["last"].Value);
            AddRange(begin, last);
        }
        ...
        {
            Helper.CheckCondition(begin <= last, "Invalid range. Begin " + begin + " must be no greater than last " + last + ".");
            long length = (long)last - (long)begin + 1;
            Helper.CheckCondition(length <= long.MaxValue, "The number of integers in the set must not be more than long.MaxValue");
        ...
        {
            Helper.CheckCondition(length > 0, "Invalid range. Must have positive length.");
        ...
        {
            Helper.CheckCondition(IsEmpty || LastElement <= lastItem, "The complete size should not be smaller than the current size");
