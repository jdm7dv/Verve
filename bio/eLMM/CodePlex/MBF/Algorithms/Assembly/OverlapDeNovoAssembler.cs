...
namespace Bio.Algorithms.Assembly
{
    ...
    public class OverlapDeNovoAssembler : IOverlapDeNovoAssembler
    {
        ...
        private static ISequence SequenceWithoutTerminalGaps(ISequence inputSequence)
        {
            ...
            Sequence seq = new Sequence(inputSequence.Alphabet, input.Substring(start, len));
            seq.ID = inputSequence.ID;
            seq.DisplayID = inputSequence.DisplayID;
            return seq;
        }
            ...
            string message)
        {
                ...
                alignment[0].AlignedSequences.Count > 0)
            {
                ...
                try
                {
                    score = (int)alignment[0].AlignedSequences[0].Metadata["Score"];
                }
                catch
                {
                    // no impl.
                ...
                if (score > bestScore.OverlapScore)
                {
                    ...
                    try
                    {
                        offsets = (int)alignment[0].AlignedSequences[0].Metadata["FirstOffset"];
                    }
                    catch
                    {
                        // no impl.
                    ...
                    try
                    {
                        offsets = (int)alignment[0].AlignedSequences[0].Metadata["SecondOffset"];
                    }
                    catch
                    {
                        // no impl.
        ...
        private class PoolItem
        {
            ...
            internal Contig Contig
            {
                get
                {
                    if (!_isContig)
                    {
                        string message = "PoolItem.Contig: Not a contig item.";
            ...
            internal ISequence Sequence
            {
                get
                {
                    if (_isContig)
                    {
                        string message = "PoolItem.Contig: Item is contig, not sequence.";
                        Trace.Report(message);
                        throw new Exception(message);
                    }
