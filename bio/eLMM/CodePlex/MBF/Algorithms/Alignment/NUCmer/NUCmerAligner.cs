...
namespace Bio.Algorithms.Alignment
{
    ...
    public class NUCmerAligner
    {
        ...
        /// </summary>
        private const int DefaultSubstitutionScore = -7;
        ...
        /// </summary>
        internal int SubstitutionScore { get; set; }
        ...
        /// </summary>
        private int GapOpeningScore { get; set; }
        ...
        internal bool ExtendSequence(
                ...
                int methodName)
        {
            IList<Diagonal> diagonals;
            bool isSequenceExtended;
            int referencePointer, queryPointer, currentDiagonal, 
            ...
            int left = 0;
            int right = 0;
            int targetHighScoreDiagonal = 0;
            int targetHighScoreDiagonalIndex = 0;
            int targetDiagonal = 0;
            int targetDiagonalIndex = 0;
            int row, column, lowerIndex;
                ...
                currentDiagonal++)
            {
                ...
                for (conceptualDiagonal = left; conceptualDiagonal <= right; conceptualDiagonal++)
                {
                    ...
                    if (previousDiagonalIndex >= 0 && previousDiagonalIndex < previousDiagonalSize)
                    {
                        deleteValue = diagonals[previousDiagonal].Nodes[previousDiagonalIndex].Scores[DeleteState].State == NoneState
                                ...
                                : diagonals[previousDiagonal].Nodes[previousDiagonalIndex].Scores[DeleteState].Value + SubstitutionScore;
                    ...
                    if (previousDiagonalIndex >= 0 && previousDiagonalIndex < previousDiagonalSize)
                    {
                        ...
                        insertValue = diagonals[previousDiagonal].Nodes[previousDiagonalIndex].Scores[InsertState].State == NoneState
                            ? diagonals[previousDiagonal].Nodes[previousDiagonalIndex].Scores[InsertState].Value
                            : diagonals[previousDiagonal].Nodes[previousDiagonalIndex].Scores[InsertState].Value + SubstitutionScore;
                        matchValue = diagonals[previousDiagonal].Nodes[previousDiagonalIndex].Scores[MatchState].State == NoneState
                            ? diagonals[previousDiagonal].Nodes[previousDiagonalIndex].Scores[MatchState].Value
                            : diagonals[previousDiagonal].Nodes[previousDiagonalIndex].Scores[MatchState].Value + GapOpeningScore;
                        SetScore(
                ...
                if ((0 < (methodName & SeqendFlag)) && currentDiagonal >= lowerIndex)
                {
                    ...
                    else
                    {
                        if (right == column)
                        {
                            if (diagonals[currentDiagonal].Nodes[column - diagonals[currentDiagonal].Left].
                                 MaximumScore.Value >= nonOptimalScore)
                            {
                                nonOptimalScore = diagonals[currentDiagonal].Nodes[column - diagonals[currentDiagonal].Left].
                                    MaximumScore.Value;
                                targetDiagonal = currentDiagonal;
            ...
            // Final index
            int referenceAdjustment = targetHighScoreDiagonal <= row ? targetHighScoreDiagonal - targetHighScoreDiagonalIndex - 1 : row - targetHighScoreDiagonalIndex - 1;
            int queryAdjustment = targetHighScoreDiagonal <= row ? targetHighScoreDiagonalIndex - 1 : targetHighScoreDiagonal - row + targetHighScoreDiagonalIndex - 1;
            if (0 < (~methodName & DirectionFlag))
            {
        ...
        private static Score GetMaximumScore(IList<Score> scores)
        {
            if (scores[DeleteState].Value > scores[InsertState].Value)
            {
                if (scores[DeleteState].Value > scores[MatchState].Value)
                {
                    return scores[DeleteState];
                }
                else
                {
                    return scores[MatchState];
                }
            }
            else if (scores[InsertState].Value > scores[MatchState].Value)
            {
                return scores[InsertState];
            }
            else
            {
                return scores[MatchState];
        ...
        {
            int deltaCounter;
            int diagonalIndex = counter;
            int conceptualDiagonal = currentDiagonal;
            int nodeIndex = 0;
            int pathIndex = 0;
            int edit;
            ...
            edit = diagonals[diagonalIndex].Nodes[nodeIndex].Scores.IndexOf(diagonals[diagonalIndex].Nodes[nodeIndex].MaximumScore);
                ...
                int methodName)
        {
            ...
            {
                referenceCharacter = referenceSequence[referenceIndex + ((diagonalCounter - diagonalIndex) * direction)].Symbol;
                queryCharacter = querySequence[queryIndex + (diagonalIndex * direction)].Symbol;
            }
            else
            {
                referenceCharacter = referenceSequence[referenceIndex + ((length - diagonalIndex) * direction)].Symbol;
                queryCharacter = querySequence[queryIndex + ((diagonalCounter - length + diagonalIndex) * direction)].Symbol;
            }
            ...
            return SimilarityMatrix[Char.ToUpper(referenceCharacter, CultureInfo.InvariantCulture) - 'A',
                    Char.ToUpper(queryCharacter, CultureInfo.InvariantCulture) - 'A'];
        }
    }
    ...
    internal class Node
    {
        ...
        /// </summary>
        internal IList<Score> Scores { get; set; }
