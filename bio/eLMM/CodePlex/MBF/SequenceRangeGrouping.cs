...
namespace Bio
{
    ...
    {
        private Dictionary<string, List<ISequenceRange>> _groups
        ...
        {
            get { return _groups.Values.SelectMany(sr => sr); }
        }
        ...
        {
            if (!_groups.ContainsKey(rangeID))
                return null;

            return _groups[rangeID];
        }
        ...
        public List<ISequenceRange> Flatten()
        {
            ...
            foreach (List<ISequenceRange> ranges in _groups.Values)
            {
                foreach (ISequenceRange range in ranges)
                {
                    result.Add(range);
        ...
        public SequenceRangeGrouping MergeOverlaps(SequenceRangeGrouping query, long minOverlap = 0, bool isParentSeqRangesRequired = false)
        {
            if (query == null)
            {
                throw new ArgumentNullException(Resource.ParameterNameQuery);
            }
