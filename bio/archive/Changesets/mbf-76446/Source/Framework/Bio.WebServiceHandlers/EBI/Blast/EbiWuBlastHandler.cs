...
namespace Bio.Web.Blast
{
    ...
    public class EbiWuBlastHandler : IBlastServiceHandler, IDisposable
    {
        ...
        /// </summary>
        private WSWUBlastService blastClient;
        ...
        public ServiceRequestInformation GetRequestStatus(string requestIdentifier)
        {
            ...
            status.StatusInformation = blastClient.checkStatus(requestIdentifier);
        ...
        {
            WSFile[] resultTypes = blastClient.getResults(requestIdentifier);
            ...
            foreach (WSFile resultType in resultTypes)
            {
                ...
                {
                    byte[] content = blastClient.poll(requestIdentifier, resultType.type);
        ...
        public IList<string> GetServiceMetadata(string kind)
        {
            ...
            switch (kind)
            {
                case MetadataDatabases:
                    data = blastClient.getDatabases();
                    break;

                case MetadataFilter:
                    data = blastClient.getFilters();
                    break;

                case MetadataMatrices:
                    data = blastClient.getMatrices();
                    break;

                case MetadataPrograms:
                    data = blastClient.getPrograms();
                    break;

                case MetadataSensitivity:
                    data = blastClient.getSensitivity();
                    break;

                case MetadataSort:
                    data = blastClient.getSort();
                    break;

                case MetadataStatistics:
                    data = blastClient.getStats();
                    break;

                case MetadataXmlFormats:
                    data = blastClient.getXmlFormats();
                    break;
            }
        ...
        {
            blastClient = new WSWUBlastService();
        }
