...
namespace Bio.Algorithms.SuffixTree
{
    ...
    public class MultiWaySuffixTree : ISuffixTree
    {
        ...
        public MultiWaySuffixTree(Sequence sequence)
        {
            ...
            if (sequence.Count == 0)
            {
                throw new ArgumentOutOfRangeException("sequence", Properties.Resource.EmptySequence);
            }
        ...
        /// </summary>
        public long EdgesCount { get; private set; }
            ...
            for (queryIndex = 0; queryIndex <= querySequenceLength - minLengthOfMatch; queryIndex++)
            {
                ...
                if (lengthOfMatchInEdge > 0)
                {
                    ...
                    while (!edge.IsLeaf)
                    {
                        ...
                        else if (lengthOfMatchInEdge > edgeSymbolCount)
                        {
        ...
        {
            // holds the possible overlapping matches, 
            // 
            // Note: new Match will be considered overlapped only when it is a substring of any of the match in the list.
            //  For example,
            //                  refindex    query index   length of match
            //      Match1      4100        2071            13    
            //      Match2      3907        2073            10
            //      Match3      4103        2074            10
            // in the above matches Match1 and Match3 are overlapping, but Match1 and Match2 are not.
            //  So the Match3 is ignored,
            // 1. When to Add a match to the list
            //     when a new match found which is not overlapping with any item of the list then it will be added to the list
            //     For example, consider the Match1 (above) is in the list and when Match2 is found at index 2073 as it is not
            //     overlapping with Match1 it is added to the list.
            // 2. When to remove match from list
            //    A matche in this list will be removed once the query index of new match is
            //    more than the (query index + match length) of the matche in the list.
            //    For example, If a new Match (4300, 2086, 10) found then both Match1 and Match2 will be removed from the list.
            List<Match> possibleOverlappingMatches = new List<Match>();
            ...
            for (queryIndex = 0; queryIndex <= querySequenceLength - minLengthOfMatch; queryIndex++)
            {
                ...
                if (suffixLink.StartIndex == previousIntermediateEdge.StartIndex - 1 && previousIntermediateEdge.SuffixLink[0].StartIndex != -1)
                {
                    ...
                    for (int suffixLinkChildIndex = 0; suffixLinkChildIndex < suffixLinkChildCount; suffixLinkChildIndex++)
                    {
                ...
                while (continueSearch)
                {
                    ...
                    if (edgeIndex == -1)
                    {
                        ...
                        {
                            // Set -1 so that it wont match with start index of any child edge.
                            edgesFound.Push(new EdgesFound() { Edge = previousIntermediateEdge, LengthOfMatch = lengthOfMatchFound });
                            childStartIndexToSkip = -1;
                        }
                    }
                    else
                ...
                if (edgesFound.Count > 0)
                {
                    ...
                    if (edge.IsLeaf)
                    {
                        referenceOffset = edge.StartIndex + lengthOfMatchInEdge - matchLength;

                        bool isoverlapedMatchFound = false;
                        for (int listIndex = possibleOverlappingMatches.Count - 1; listIndex >= 0; listIndex--)
                        {
                            Match lastMatch = possibleOverlappingMatches[listIndex];

                            if (lastMatch.QuerySequenceOffset <= queryIndex
                                && lastMatch.QuerySequenceOffset + lastMatch.Length >= queryIndex + matchLength
                                && lastMatch.ReferenceSequenceOffset <= referenceOffset
                                && lastMatch.ReferenceSequenceOffset + lastMatch.Length >= referenceOffset + matchLength
                                && (queryIndex - lastMatch.QuerySequenceOffset) == (referenceOffset - lastMatch.ReferenceSequenceOffset))
                            {
                                isoverlapedMatchFound = true;
                                break;
                            }
                            else
                                if ((lastMatch.QuerySequenceOffset + lastMatch.Length) < queryOffset)
                                {
                                    possibleOverlappingMatches.RemoveAt(listIndex);
                                }
                        }
                        ...
                        {
                            match = new Match();
                            match.ReferenceSequenceOffset = referenceOffset;
                            match.QuerySequenceOffset = queryOffset;
                            match.Length = matchLength;
                            possibleOverlappingMatches.Add(match);
                            yield return match;
                        }
                    }
                    else
                    {
                        ...
                        for (int matchIndex = 0; matchIndex < listCount; matchIndex++)
                        {
                            ...
                            referenceOffset = startIndex + edgeLengthToAdd - matchLength;
                            bool isoverlapedMatchFound = false;
                            for (int listIndex = possibleOverlappingMatches.Count - 1; listIndex >= 0; listIndex--)
                            {
                                Match lastMatch = possibleOverlappingMatches[listIndex];
                    ...
                    while (edgesFound.Count > 0)
                    {
                        ...
                        if (!edge.IsLeaf &&
                            !previousItemToDisplay.Edge.IsLeaf &&
                            previousItemToDisplay.Edge.StartIndex != previousItemToDisplay.Edge.SuffixLink[0].StartIndex)
                        {
                        ...
                        if (matchLength - 1 >= minLengthOfMatch)
                        {
                            ...
                            edgesFoundForNextQueryIndex.Push(new EdgesFound() { Edge = edge.SuffixLink[0], LengthOfMatch = matchLength - 1 });
                        }
                        ...
                        {
                            long startIndex = startIndexes[matchIndex];
                            referenceOffset = startIndex + edgeLength - matchLength;
                            bool isoverlapedMatchFound = false;
                            for (int listIndex = possibleOverlappingMatches.Count - 1; listIndex >= 0; listIndex--)
                            {
                                Match lastMatch = possibleOverlappingMatches[listIndex];
            ...
            else
            {
                ...
                while (!done)
                {
                    ...
                    for (; childIndex < count; childIndex++)
                    {
                        ...
                        {
                            stack.Push(new Tuple<MultiWaySuffixEdge, byte, long>(current, (byte)(childIndex + 1), length));
                                ...
                                if (edge.StartIndex < edgeEndIndex)
                                {
                                    ...
                                    for (long counter = edge.StartIndex + 1; counter <= edgeEndIndex; counter++, startIndex++)
                                    {
                                        ...
                                        if (symbolAtStartIndex != symbolAtCounter)
                                        {
                                            ...
                                            {
                                                for (int duplicatedIndex = 1; duplicatedIndex < duplicatedConsicutiveSymbolsCount; duplicatedIndex++)
                                                {
                                                    MultiWaySuffixEdge duplicateSymbolEdge = new MultiWaySuffixEdge(newEdge.StartIndex - 1);
        ...
        private Stack<EdgesFound> GetIntermediateEdges(MultiWaySuffixEdge fromEdge, MultiWaySuffixEdge toedge, long matchLengthOfFromEdge, long lengthToSearch, long nextQueryIndex, Sequence convertedSearchSeq, long minLengthOfMatch)
        {
            ...
            if (lengthToSearch > 0)
            {
                ...
                while (!edge.IsLeaf && edge.StartIndex != childIndexToStop)
                {
                    ...
                    else if (lengthToSearch > edgeSymbolCount)
                    {
                        ...
                        for (int edgeChildIndex = 0; edgeChildIndex < edgeChildCount; edgeChildIndex++)
                        {
                            if (this.referenceSequence[edge.Children[edgeChildIndex].StartIndex] == querySymbol)
                            {
                                ...
                                {
                                    edgesFoundForNextQueryIndex.Push(new EdgesFound() { Edge = previousIntermediateEdge, LengthOfMatch = matchLengthOfFromEdge });
                                }
